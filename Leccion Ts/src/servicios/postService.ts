...
export async function obtenerPostPorId(id: number): Promise<Post> {
...
export async function filtrarPostsPorUsuario(userId: number): Promise<Post[]> {
...
export async function crearPost(post: Omit<Post, "id">): Promise<Post> {
  const respuesta = await fetch(API_URL, {
    method: "POST",
    headers: {
...
export async function actualizarTituloPost(id: number, titulo: string): Promise<Post> {
  const respuesta = await fetch(`${API_URL}/${id}`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json"
